...
import { CuestionarioCompuestoBack, CuestionarioCompuestoRespuesta, CuestionarioSimpleRespuesta } from "../interfaces/cuestionario";
...
export class CuestionarioService {
    ...
    private files = [] as Array<{control: string, fileId: number, justificacion: string | null}>;
    ...
    setFirmaDatosAdicionales(array: Array<any> | null) {
        if(array == null) {
            ...
            if(this.firmaDatosAdicionales.length == 0) {
                ...
                this.firmaDatosAdicionales = this.firmaDatosAdicionales.filter(x => x.id_pregunta != array[0].id_pregunta);
                array.forEach(dato => {
                    this.firmaDatosAdicionales.push(dato);
                })
            }
    ...
    getListaById(idLista: number): Observable<any> {
        return this.http.get<any>(`${environment.base_url}ListaDatos?id=` + idLista)
    }
