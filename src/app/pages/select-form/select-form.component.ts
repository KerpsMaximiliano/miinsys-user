...
export class SelectFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log(
      this.menuTree.empresasDto.find(
        (e: { emp_id: number }) =>
          e.emp_id == this.rutaActiva.snapshot.params['empresaId']
      )
    );
    ...
    this.plantasService.getPlantas(this.companyID).subscribe((d) => {
      if (d.length > 0) {
        this.nombrePlanta = d.find(
          (pl: { id_planta: number }) => pl.id_planta == this.plantId
        ).descripcion;
      }
    });
    if (this.menuTree.empresasDto.length > 1) {
      this.returnUrl =
        'dashboard/selectGroup/' + this.rutaActiva.snapshot.params['empresaId'];
    }
    this.nombreEmpresa = this.menuTree.empresasDto.find(
      (e: { emp_id: number }) =>
        e.emp_id == this.rutaActiva.snapshot.params['empresaId']
    ).emp_descripcion;
    if (this.plantId != 0) {
      this.menuTree.empresasDto[0].centroTrabajo =
        this.menuTree.empresasDto[0].centroTrabajo.filter(
          (x: { cuestionario: { id_planta: number } }) =>
            x.cuestionario.id_planta == this.plantId
        );
    }
    console.log(this.nombreGrupo);
    if (this.menuTree.empresasDto.length == 0) {
    } else {
      this.menuTree.empresasDto.forEach(
        (emp: { emp_id: any; centroTrabajo: any[] }) => {
...
@Component({
  ...
  template: `<h1 mat-dialog-title>Permisos de ubicación bloqueados</h1>
    <div mat-dialog-content>
      ...
      <img
        src="../assets/images/location-guide/print 1 - trimmed.jpg"
        alt=""
        style="max-width: 65vw"
        class="mb-4"
      />
      <img
        src="../assets/images/location-guide/print 2 - trimmed.jpg"
        alt=""
        style="max-width: 65vw"
        class="mb-4"
      />
      <img
        src="../assets/images/location-guide/print 3 - trimmed.jpg"
        alt=""
        style="max-width: 65vw"
        class="mb-4"
      />
    ...
    <div mat-dialog-actions>
      <button mat-button class="main__input--send" (click)="dialogRef.close()">
        Cerrar
      </button>
    </div>`,
})
export class LocationErrorModalComponent implements OnInit {
  constructor(public dialogRef: MatDialogRef<LocationErrorModalComponent>) {}
