...
export class CuestionarioTemplateComponent implements OnInit, AfterViewInit {
  ...
  currentUser: string = '';
  currentUsername: {nombre: string, apellido: string} = {nombre: "", apellido: ""};
  latitude: number = 0;
  longitude: number = 0;
  autos = [
  ...
  */
  ...
  ngOnInit(): void {
    this.loading = true;
    this.currentUser = JSON.parse(sessionStorage.getItem('usuarioLogueado')!).username.toString();
    console.log(JSON.parse(sessionStorage.getItem('usuarioLogueado')!));
    this.currentUsername.nombre = JSON.parse(sessionStorage.getItem('usuarioDatos')!).firstName;
    this.currentUsername.apellido = JSON.parse(sessionStorage.getItem('usuarioDatos')!).lastName;
    ...
    if (Object.keys(this.cuestionarioService.getAnswerBack()).length !== 0) {
      ...
      this.cuestionarioBack.forEach((seccion: { preguntas: { tipo: number; control: any; justificado: any; opciones: any; lst_id: number}[]; nombre: string;}, index: number) => {
        let check = false;
        seccion.preguntas.forEach((p) => {
          if(p.tipo === 10 || p.tipo === 15) {
            console.log(seccion)
            let find = seccion.preguntas.find(pr => pr.control == p.control)!.opciones.find((op: { id: any; }) => op.id == this.cuestionarioForm.get(p.control)!.value);
            console.log(find);
            if(find != undefined) {
              if(find.valor) {
                if(find.valor === 0) {
                  this.cuestionarioForm.get(p.control)!.setValue((p.justificado ? find.id : ''));
                };
              };
            }
          };
      ...
      if(this.controlListaIds.length > 0) {
        this.controlListaIds.forEach(controlId => {
          this.cuestionarioService.getListaById(controlId.id).subscribe(resLista => {
            this.listas[controlId.id] = resLista;
            let modelListaMostrar = [] as Array<any>;
            let find = this.listas[controlId.id].lista.find((lst: { id_fila: any; }) => lst.id_fila == Number(this.cuestionarioForm.get(controlId.control)?.value));
            ...
            if(this.controlListaIds.indexOf(controlId) == (this.controlListaIds.length - 1)) {
      ...
      setTimeout(() => {
        if(Object.keys(this.cuestionarioService.getAnswerBack()).length !== 0) {
          ...
          document.getElementById(this.cuestionarioService.getAnswerBack().seccion.nombre)?.scrollIntoView();
          }
      }, 1);
    } else {
      this.usuarioService.getCuestionario(this.cuestionarioId).subscribe(d => {
        ...
        d.secciones.forEach((seccion: { sec_id: any; sec_descripcion: any; orden: any; preguntas: any[]; }, index: number) => {
          let seccionesForm = {
          ...
          seccion.preguntas.forEach(pregunta => {
            ...
            if(preguntaForm.tipo === 4) {
              this.cuestionarioForm.addControl(preguntaForm.control, new FormControl({value: pregunta.opciones[0].opc_descripcion, disabled: true}));
              this.answeredQuestions++;
            } else if(preguntaForm.tipo === 8) { //Nombre de usuario
              this.cuestionarioForm.addControl(preguntaForm.control, new FormControl({value: this.currentUsername.apellido + ', ' + this.currentUsername.nombre, disabled: true}));
              this.answeredQuestions++;
            } else if (preguntaForm.tipo === 9) { //Fecha y hora
              let currentDate = new Date();
              this.cuestionarioForm.addControl(preguntaForm.control, new FormControl({value:`${currentDate.getDate()}-${currentDate.getMonth() + 1}-${currentDate.getFullYear()}, ${currentDate.getHours()}:${currentDate.getMinutes()}`, disabled: true}));
              this.answeredQuestions++;
            } else if (preguntaForm.tipo === 11) { //RUT_USUARIO
              this.cuestionarioForm.addControl(preguntaForm.control, new FormControl({value: this.currentUser, disabled: true}));
              this.answeredQuestions++;
            } else {
              if(preguntaForm.tipo === 5) {
                //Texto
                this.cuestionarioForm.addControl(preguntaForm.control, new FormControl('', [Validators.required, Validators.pattern('^[ a-zA-ZÀ-ÿ\u00f1\u00d1]*$')]));
              } else if(preguntaForm.tipo === 7) {
                //Numero
                this.cuestionarioForm.addControl(preguntaForm.control, new FormControl('', [Validators.required, Validators.pattern('(0|[1-9][0-9]*)$')]));
              } else if(preguntaForm.tipo === 17) {
                //Alfanumérico
                this.cuestionarioForm.addControl(preguntaForm.control, new FormControl('', [Validators.required]));
              ...
              if(preguntaForm.validators.length > 0) {
                preguntaForm.validators.forEach((val: { id_validacion: any; Value: string; }) => {
                  switch (val.id_validacion) {
                    case 0:
                      this.cuestionarioForm.get(preguntaForm.control)!.addValidators(Validators.required);
                      break;
                    case 1:
                      this.cuestionarioForm.get(preguntaForm.control)!.addValidators(Validators.max(Number(val.Value)));
                      break;
                    case 2:
                      this.cuestionarioForm.get(preguntaForm.control)!.addValidators(Validators.min(Number(val.Value)));
                      break;
                    case 3:
                      this.cuestionarioForm.get(preguntaForm.control)!.addValidators(Validators.minLength(Number(val.Value)));
                      break;
                    case 4:
                      this.cuestionarioForm.get(preguntaForm.control)!.addValidators(Validators.maxLength(Number(val.Value)));
                      break;
                  }
                })
              } else {
                this.cuestionarioForm.get(preguntaForm.control)?.removeValidators(Validators.required);
              }
            };
            if(this.cuestionarioId == 4) {
              switch (true) {
                case preguntaForm.preguntaId == 75:
                  this.cuestionarioForm.controls[preguntaForm.control].addValidators([Validators.min(0), Validators.max(10)]);
                  break;
                case preguntaForm.preguntaId == 76 || preguntaForm.preguntaId == 77:
                  this.cuestionarioForm.controls[preguntaForm.control].addValidators([Validators.min(0), Validators.max(2)]);
                  break;
                case (preguntaForm.preguntaId >= 78 && preguntaForm.preguntaId <= 87) || preguntaForm.preguntaId == 163:
                  this.cuestionarioForm.controls[preguntaForm.control].addValidators([Validators.max(128)]);
                  break;
                case preguntaForm.preguntaId == 88 || preguntaForm.preguntaId == 162 || preguntaForm.preguntaId == 164:
                  this.cuestionarioForm.controls[preguntaForm.control].addValidators([Validators.maxLength(128)]);
                  break;
              }
            };
            pregunta.opciones.forEach((opcion: { opc_id: any; opc_descripcion: any; opc_valor: any; opc_orden: number}) => {
              let opcionesForm = {
          ...
          this.panelClassesBack.push({position: index, class: "panel-white"});
        })
        this.cuestionarioBack = this.cuestionarioBack.sort((seccion1: { orden: number; }, seccion2: { orden: number; }) => (seccion1.orden < seccion2.orden) ? -1 : (seccion1.orden > seccion2.orden) ? 1 : 0);
        ...
        this.cuestionarioBack.forEach((cues: { preguntas: any[]; }) => {
          if(cues.preguntas.length > 0) {
            cues.preguntas.sort((a: any, b: any) => {
              return (a.orden < b.orden) ? -1 : (a.orden > b.orden) ? 1 : (a.label.toLowerCase() > b.label.toLowerCase()) ? 1 : (a.label.toLowerCase() < b.label.toLowerCase()) ? -1 : 0;
            });
          }
        })
        ...
        this.totalQuestions = Object.keys(this.cuestionarioForm.controls).length - this.answeredQuestions;
        this.answeredQuestions = 0;
        if(this.listaIds.length > 0) {
          this.listaIds.forEach(id => {
            this.cuestionarioService.getListaById(id).subscribe(resLista => {
        ...
        this.cuestionarioForm.valueChanges.subscribe(d => {
          ...
          let porcentaje = (100 / this.totalQuestions) * this.answeredQuestions;
          this.progressBarWidth = `calc(${porcentaje}% - 2px)`;
          if(document.getElementById('inner-progress-bar')) {
            document.getElementById('inner-progress-bar')!.style.width = `calc(${porcentaje}% - 2px)`;
          }
        });
        this.cuestionarioForm.updateValueAndValidity();
        console.log(this.cuestionarioBack)
  ...
  finalizarFormulario() {
    if(this.cuestionarioForm.invalid) {
      this.openSnackBar("Debe completar todos los campos y contestar todas las preguntas", "Cerrar", "warn-snackbar");
      return
    };
    ...
    if(this.cuestionarioService.getFirmaDatosAdicionales().length > 0) {
      Object.assign(model, {firmaDatosAdicionales: this.cuestionarioService.getFirmaDatosAdicionales()});
    };
    this.cuestionarioBack.forEach((seccion: { seccionId: any; preguntas: { preguntaId: any; tipo: any; control: any; opciones: [{
      valor: any; descripcion: any; id: any;
    }]}[]; }) => {
      let seccionEnviar = {
        seccionId: seccion.seccionId,
        preguntas: [] as Array<{}>
      };
      seccion.preguntas.forEach((pregunta: { preguntaId: any; tipo: any; control: any; opciones: [{
        valor: any; descripcion: any; id: any;
      }]}) => {
        console.log(pregunta)
        let preguntaEnviar = {
          ...
          preguntaValor: this.cuestionarioForm.get(pregunta.control)!.value.toString(),
          preguntaOpcion: 0
        };
        if(preguntaEnviar.preguntaTipo === 10 || preguntaEnviar.preguntaTipo === 15) {
          preguntaEnviar.preguntaOpcion = this.cuestionarioForm.get(pregunta.control)!.value;
          console.log(seccion)
          let findOpc = pregunta.opciones.find(opc => opc.id == preguntaEnviar.preguntaOpcion);
          preguntaEnviar.preguntaValor = findOpc?.valor.toString();
          if(findOpc!.valor == 0) {
            let find = this.cuestionarioService.getFiles().find(file => file.control == pregunta.control);
            console.log(find)
            Object.assign(preguntaEnviar, {preguntaImagenId: find?.fileId});
            Object.assign(preguntaEnviar, {justificacion_imagen: find?.justificacion});
          }
        } else if(preguntaEnviar.preguntaTipo === 14) {
            let find = this.cuestionarioService.getFiles().find(file => file.control == pregunta.control);
            Object.assign(preguntaEnviar, {preguntaImagenId: find?.fileId});
            preguntaEnviar.preguntaOpcion = pregunta.opciones[0].id;
            console.log(this.cuestionarioForm.get(pregunta.control)!.value.toString())
            preguntaEnviar.preguntaValor = this.cuestionarioForm.get(pregunta.control)!.value.toString();
          ...
          preguntaEnviar.preguntaOpcion = pregunta.opciones.find(op => op.descripcion == this.cuestionarioForm.get(pregunta.control)!.value)!.id;
        } else if (preguntaEnviar.preguntaTipo == 12) {
          let date = new Date(preguntaEnviar.preguntaValor);
          preguntaEnviar.preguntaValor = `${date.getDate()}-${date.getMonth() + 1}-${date.getFullYear()}`;
          preguntaEnviar.preguntaOpcion = pregunta.opciones[0].id
        } else {
          preguntaEnviar.preguntaOpcion = pregunta.opciones[0].id
        }
    ...
    model.secciones.forEach(sec => {
      sec.preguntas.forEach((pre: { preguntaTipo: number; preguntaValor: any; }) => {
    ...
    this.cuestionarioService.setScores({positive: scorePositive, negative: scoreNegative});
  ...
  radioEventBack(event: any, control: string, seccion: any, index: number) {
    ...
    console.log(control);
    console.log(index);
    let find = seccion.preguntas.find((pre: { control: string; }) => pre.control == control).opciones.find((op: { id: any; }) => op.id == event.value);
    if(find.valor == 0) {
      this.panelCloseBack(seccion, index);
      let titulo = seccion.preguntas.find((pre: { control: string; }) => pre.control == control).label;
      let model = {
        titulo: this.cuestionarioBackTitle,
        formGroup: this.cuestionarioForm,
        respuestas: this.cuestionarioBack,
      ...
      this.router.navigate(["dashboard/justifyAnswer"], { queryParams: { returnUrl: this.router.url } });
    }
  }
  ...
  panelClose(index: number) {
    ...
    if(index == 0) {
      if(this.cuestionarioForm.get('pregunta1')?.valid && this.cuestionarioForm.get('pregunta2')?.valid && this.cuestionarioForm.get('pregunta3')?.valid) {
        this.panelClasses[index].class = 'panel-green';
      } else {
        this.panelClasses[index].class = 'panel-red';
      }
    } else if (index == 1){
      if(this.cuestionarioForm.get('pregunta4')?.valid && this.cuestionarioForm.get('pregunta5')?.valid && this.cuestionarioForm.get('pregunta6')?.valid) {
        this.panelClasses[index].class = 'panel-green';
      } else {
        this.panelClasses[index].class = 'panel-red';
      }
    } else {
      if(this.cuestionarioForm.get('fecha')?.valid && this.cuestionarioForm.get('hora')?.valid) {
        this.panelClasses[index].class = 'panel-green';
      } else {
        this.panelClasses[index].class = 'panel-red';
      }
  ...
  panelCloseBack(seccion: any, index: number) {
    ...
    seccion.preguntas.forEach((pregunta: { control: any; }) => {
      if(this.cuestionarioForm.get(pregunta.control)?.valid || this.cuestionarioForm.get(pregunta.control)?.disabled) {
        return;
      } else {
        error = true;
      }
  ...
  openFoto(control: string) {
    let fileId = this.cuestionarioService.getFiles().find(file => file.control == control)!.fileId;
      ...
      if(extension == "heic" || extension == "heif") {
        ...
        convProm = heic2any({blob,toType:"image/jpeg",quality:0.2}).then((jpgBlob:Blob | any) => {
          ...
          if(event.srcElement.files.length > 0) {
            let find = this.cuestionarioService.getFiles().find(file => file.control == control);
            if(find == undefined) {
              this.fileService.subirArchivo(finalFile, 1).subscribe(d => {
                this.cuestionarioService.setFile({control: control, fileId: d.id, justificacion: ""});
                this.cuestionarioForm.get(control)?.setValue(finalFile.name);
                this.openSnackBar(d.message, "X", "success-snackbar");
              });
            } else {
              this.fileService.actualizarArchivo(finalFile, find.fileId, 1).subscribe(d => {
                this.cuestionarioService.setFile({control: control, fileId: d.id, justificacion: ""});
                this.cuestionarioForm.get(control)?.setValue(finalFile.name);
                this.openSnackBar(d.message, "X", "success-snackbar");
              });
        ...
        if(event.srcElement.files.length > 0) {
          ...
          let find = this.cuestionarioService.getFiles().find(file => file.control == control);
          if(find == undefined) {
            this.fileService.subirArchivo(event.srcElement.files[0], 1).subscribe(d => {
              this.cuestionarioForm.get(control)?.setValue(name);
              this.cuestionarioService.setFile({control: control, fileId: d.id, justificacion: ""});
              this.openSnackBar(d.message, "X", "success-snackbar");
            });
          } else {
            this.fileService.actualizarArchivo(event.srcElement.files[0], find.fileId, 1).subscribe(d => {
              this.cuestionarioForm.get(control)?.setValue(name);
              this.cuestionarioService.setFile({control: control, fileId: d.id, justificacion: ""});
              this.openSnackBar(d.message, "X", "success-snackbar");
            });
  ...
  sign(event: any, control: string, seccion: any, index: number) {
    ...
    this.cuestionarioService.setQuestionTitle(titulo);
    setTimeout(() => {
      this.router.navigate(["dashboard/preguntaFirma"], { queryParams: { returnUrl: this.router.url } });
    }, 20);
  }

  selectLista(event: any, listaId: number, ) {
    let modelListaMostrar = [] as Array<any>;
    let find = this.listas[listaId].lista.find((lst: { id_fila: any; }) => lst.id_fila == event.value);
    for (const [key, value] of Object.entries(find.descripciones)) {
      modelListaMostrar.push(value)
    };
...
@Component({
  ...
  template: `<div>
  <img [src]="imgSrc" class="ml-auto mr-auto rounded" style="max-height: 70vh; max-width: 70vw" alt="" *ngIf="imgSrc != '' && type == 'image'">
  <video [src]="imgSrc" controls style="max-height: 70vh; max-width: 70vw" *ngIf="imgSrc != '' && type == 'video'"></video>
      ...
      this.fileService.traerArchivo(this.data.id, 1).subscribe(d => {
        ...
        if(this.fileService.getFileType(d.fileName) == 'image/jpeg') {
          ...
          if(this.data.additionalData) {
            this.cuestionarioService.getFirmaDatoAdicional(this.data.additionalData[0].id_pregunta).subscribe(d => {
              console.log(d);
