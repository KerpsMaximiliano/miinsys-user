...
export class CriticTaskComponent implements OnInit {
  ...
  getCriticTask() : CriticTaskModel[]
  {
    ...
    const workCenter : WorkCenterModel[] = estructuraCuestionarios.companyUser.find(x=>x.id==this.companyID)!.workCenters;
    const serviceType = workCenter.find(x=>x.id==this.workCenterID)!.servicesTypes; 
    const criticTask =  serviceType.find(x=>x.id==this.servicesTypeID)!.criticTasks;
    return criticTask;
  }
